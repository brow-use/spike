...
export const writeDocsIndex: Tool = {
  ...
  async execute(input, ctx: ToolContext): Promise<string> {
    ...
    lines.push('')
    lines.push(`- **Trace chunks** — replay every action with before/after screenshots, DOM snapshots, network and source: \`output/trace/${sessionId}/chunk-*.zip\``)
    lines.push(`  Open a chunk with: \`npx playwright show-trace output/trace/${sessionId}/chunk-0000.zip\``)
