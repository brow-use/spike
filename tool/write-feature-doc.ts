...
export const writeFeatureDoc: Tool = {
  ...
  description: 'Write an end-user-facing Markdown doc to output/docs/<name>.md. Use name "README" for the index. Content should be plain-language documentation for end users of the app (not developers).',
  inputSchema: {
    type: 'object',
    properties: {
  ...
  async execute(input, ctx: ToolContext): Promise<string> {
    ...
    const dir = path.join(ctx.outputDir, 'docs')
